...
class PackageManagerPacman(PackageManagerBase):
    ...
    def process_install_requests_bootstrap(self):
        ...
        bash_command = [
            ...
            '--root', self.root_dir
        ] + self.custom_args + ['-S', '--force'] + self.package_requests
    ...
    def process_install_requests(self):
        ...
        :rtype: namedtuple
        """
        # TODO handle exclude_requests
        chroot_pacman_args = self.root_bind.move_to_root(
            self.pacman_args
        )
        bash_command = [
    ...
    def process_delete_requests(self, force=False):
        ...
        self.cleanup_requests()
        chroot_pacman_args = self.root_bind.move_to_root(self.pacman_args)
        return Command.call(
            [
                'chroot', self.root_dir, 'pacman'
            ] + chroot_pacman_args + self.custom_args + [
